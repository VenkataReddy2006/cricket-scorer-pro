                        ...
                        child: TabBarView(
                          children: [
                            ScorecardInningsView(
                              ...
                              bowlingTeamIds: (_match.matchData['type'] == 'single_mode' || _match.matchData['type'] == 'pair_mode')
...
class _ScorecardInningsViewState extends State<ScorecardInningsView> {
  ...
  Widget build(BuildContext context) {
    ...
    var activeBatsmen = batsmen
        ...
        .toList();
    var activeBowlers = bowlers
        .where((p) {
          final stats = getPlayerStats(p.id);
          return (stats['bowledBalls'] ?? 0) > 0;
        })
        .toList();
    var yetToBat = batsmen.where((p) => !activeBatsmen.contains(p)).toList();

    int inningsNum = widget.teamName == widget.match.team1Name ? 1 : 2;
    List<dynamic> balls = widget.match.matchData['balls_$inningsNum'] ?? widget.match.matchData['balls'] ?? [];
    List<Map<String, dynamic>> partnerships = _calculatePartnerships(balls, allPlayers);
    ...
    final sections = ['Scorecard', 'Partnerships', 'Fall of Wickets', 'Commentary'];
